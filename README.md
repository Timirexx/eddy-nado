...
- **Chat history is stored in the browser.** There is no database and no auth to scope conversations to an account, so history lives in `localStorage`: it survives refreshes and reopening the app, but it is per-browser and per-device rather than synced. localStorage also caps around 5MB, and a pasted chart is tens of KB of base64 — when the quota is reached, `src/storage.js` sheds images from the oldest conversations first, then drops the oldest conversations, and tells the user which happened. Moving history to a real backend means adding a database plus a way to identify the user (wallet signature, say) — the storage module is the only place that would need to change.
